...
export const createMessage = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    // Atualizar conversation's updatedAt
...
// Função para dar contexto para a IA
...
// Função para mudar o título da conversa
...
// Função para criar múltiplos arquivos
...
export const renameFile = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    // Verifica se o aquivo com o o novo nome já existe no mesmo diretório
...
export const deleteFile = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    // Excluir recursivamente arquivos/pastas e todos os descendentes
    const deleteRecursive = async (fileId: typeof args.fileId) => {
      const item = await ctx.db.get(fileId);
      if (!item) return;

      // Se for um aquivo, deletar todos os decendentes primeiro
      if (item.type === "folder") {
      ...
      // Excluir arquivo armazenado se existir
      if (item.storageId) {
        await ctx.storage.delete(item.storageId);
      }

      // Deletar o arquivo/pasta em si
